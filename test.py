...
from main import parse_my_args
...
def predict_labels(clf, best_pipe, features, target):
    ...
    return balanced_accuracy_score(target.values, y_pred)
...
def find_best_classifier(classifiers, dm_reductions, scorer, X_t, y_t, X_c, y_c, X_v, y_v, cv_sets, params, jobs):
    ...
    for dm in dm_reductions:
        ...
        for clf in classifiers:
            ...
            clf, dm_reduce, train_score, test_score = train_calibrate_predict(clf = clf, dm_reduction = dm,
                                                                              X_train = X_t, y_train = y_t,
                                                                              X_calibrate = X_c, y_calibrate = y_c,
                                                                              X_test = X_v, y_test = y_v,
                                                                              cv_sets = cv_sets,
...
features = inputs.drop('label', axis = 1)
features.head(5)
# feature_details = explore_data(features, inputs, data_path)
...
RF_clf = RandomForestClassifier(n_estimators = 200, random_state = 123, class_weight = 'balanced')
AB_clf = AdaBoostClassifier(n_estimators = 200, random_state = 123)
GNB_clf = GaussianNB()
...
parameters_RF = {'clf__max_features': ['auto', 'log2'],
                 'dm_reduce__n_components': np.arange(5, feature_len, np.around(feature_len / 5),dtype=int)}
parameters_AB = {'clf__learning_rate': np.linspace(0.5, 2, 5),
                 'dm_reduce__n_components': np.arange(5, feature_len, np.around(feature_len / 5),dtype=int)}
parameters_GNB = {'dm_reduce__n_components': np.arange(5, feature_len, np.around(feature_len / 5),dtype=int)}
...
plot_training_results(clfs, dm_reductions, np.array(train_scores), np.array(test_scores), path = data_path)
...
print("The best classifier is a {} with {}.".format(best_clf.base_estimator.__class__.__name__,
                                                    best_dm_reduce.__class__.__name__))
plot_confusion_matrix(y_test, X_test, best_clf, best_dm_reduce, path = data_path, normalize = True)

# Plotting a confusion matrix of bookkeepers
plot_bookkeeper_cf_matrix(match_data, bk_cols, data_path, verbose = True, normalize = True)
